...
#[test]
fn dot_show_types_all_and_single() {
    let dir = tempfile::tempdir().unwrap();
    std::fs::create_dir_all(dir.path().join("crate_a/src")).unwrap();
    std::fs::create_dir_all(dir.path().join("crate_b/src")).unwrap();
    std::fs::write(
    ...
    cmd.args([
        ...
        "--show-types-crates",
        "crate_a",
        "--show-types-crates-all",
        "-x",
    ])
    .current_dir(dir.path());
    let out = cmd.assert().get_output().stdout.clone();
    let s = String::from_utf8_lossy(&out);
    assert_eq!(s.matches("subgraph cluster_crate_a").count(), 1);
    assert_eq!(s.matches("subgraph cluster_crate_b").count(), 1);
}
